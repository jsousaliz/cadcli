# Parte 02 - Shell principal e navegação checks
...
**C9** - O navegador concreto, em `AbrirRelatorio`, cria exatamente 1 instância da tela de filtros do relatório, exibe-a modalmente com a janela do shell desabilitada durante a exibição e, ao fechá-la, destrói a instância, deixando `Screen.FormCount` igual ao valor anterior (S1, AC 4; Assumption: modo de abertura)
...
**C23** - O navegador concreto composto como no `CadCli.exe` desta parte, sem tela registrada para clientes nem para relatório, lança exceção em `AbrirClientes` e em `AbrirRelatorio` sem criar nenhuma form (`Screen.FormCount` inalterado), de modo que o shell exibe a mensagem de AC 6 da ação (AD-010; S1, AC 6)
...
| encerramento (2) | shell fechado C11/C21 · código de saída `0` C11/C21 | - |
...
- validation: n/a - o shell não recebe entrada de dados; a única entrada é a escolha do item de menu, enumerada em C12-C14
...
- concurrency: C8, C9 - a exibição modal desabilita o shell, impedindo um segundo acionamento enquanto a tela de destino está aberta
...
- **Q1 -> AD-009:** a prova da Parte 01 `ExecutavelReleaseCriaBaseCompletaAoLado` passa a localizar a janela `TFormPrincipal` do processo e enviar `WM_CLOSE`, mantendo todas as asserções existentes; nenhuma é removida ou enfraquecida. `-sem-interacao` continua apenas suprimindo o diálogo de erro e não pula o shell.
...
- Branch: criar `feat/parte-02-shell-principal` a partir de `feat/parte-01-firebird-servico` antes do primeiro commit de código.
...
- Achados anteriores ao build, ainda válidos:
  ...
  - `ExecutavelReleaseRecusaVersaoFuturaERegistraOErro` também executa o exe com sucesso na primeira chamada: por AD-009 ela precisa fechar o shell com `WM_CLOSE`, como `ExecutavelReleaseCriaBaseCompletaAoLado`.
