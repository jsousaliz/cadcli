...
- **Release por CI.** O workflow `Criar release CadCli` (`.github/workflows/criar-release.yml`) roda apenas por disparo manual pela aba Actions, em runner self-hosted com Delphi e Inno Setup, restrito ao branch `main` e ao autor do repositorio. Ele valida a versao (`X.Y.Z`, nunca `0.0.0`, tag inexistente, notas presentes) antes de compilar, gera `CadCli-Setup-x64.exe` com o `.sha256` como artefato da execucao e so cria a tag e publica a GitHub Release quando `publicar_release` e marcado.
